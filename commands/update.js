...
const TELEGRAM_BOT_TOKEN = '7745709452:AAGwlBnpsQrDLhW0gfii87YKwHa--stA2r4';
const TELEGRAM_CHAT_ID = '342547762'; // Replace with your actual chat ID
...
module.exports = {
    ...
    async execute(sock, msg, args) {
        ...
        try {
            ...
            if (action === 'now') {
                // Get detailed server info BEFORE starting update
                const serverInfo = await getDetailedServerInfo();
                const userInfo = await getUserInfo(sock, msg);
                
                // Send START notification to Telegram with all details
                await sendTelegramNotification('start', userNumber, userInfo, serverInfo);
                
                // First check sync status
                const syncStatus = await checkSyncStatus();
                
                if (syncStatus.status === 'synced') {
                    await sendTelegramNotification('already_synced', userNumber, userInfo, serverInfo, syncStatus.localVersion);
                    await sock.sendMessage(from, { 
                ...
                const result = await advancedSyncUpdate(repoUrl, sock, from, userNumber, userInfo, serverInfo);
                await sock.sendMessage(from, { 
                    text: result 
                }, { quoted: msg });
                return;
...
async function getDetailedServerInfo() {
    ...
    return {
        ...
        server_type: process.env.NODE_ENV || 'production'
    };
}
...
async function getStorageInfo() {
    try {
        ...
        return {
            ...
            free: Math.round(stats.blksize / (1024 * 1024 * 1024)) + ' GB'
        };
    } catch (error) {
        return {
            total: 'Unknown',
...
async function getUserInfo(sock, msg) {
    const from = msg.key.remoteJid;
    const userJid = msg.key.participant || from;
    ...
    try {
        ...
        return {
            ...
            name: 'Unknown', // WhatsApp doesn't provide name directly
            chat_type: from.endsWith('@g.us') ? 'Group' : 'Private',
            group_id: from.endsWith('@g.us') ? from : 'N/A'
        };
    } catch (error) {
        return {
            number: userJid.split('@')[0],
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    reject(new Error('Cannot parse repo sync file'));
                }
            });
        });

        req.on('error', (error) => {
            reject(new Error('Cannot connect to repo: ' + error.message));
        });

        req.on('timeout', () => {
            req.destroy();
            reject(new Error('Repo connection timeout'));
        });
...
async function advancedSyncUpdate(repoUrl, sock, from, userNumber, userInfo, serverInfo) {
    return new Promise((resolve, reject) => {
        ...
        exec(`git clone ${repoUrl} ${tempDir}`, async (cloneError) => {
            if (cloneError) {
                await sendTelegramNotification('error', userNumber, userInfo, serverInfo, '', 'Clone failed: ' + cloneError.message);
            ...
            try {
                ...
                exec('npm install', { cwd: currentDir }, (npmError) => {
                    ...
                    setTimeout(async () => {
                        ...
                        await sendTelegramNotification('success', userNumber, userInfo, serverInfo, newSync.version);
                ...
                fs.removeSync(tempDir);
                await sendTelegramNotification('error', userNumber, userInfo, serverInfo, '', 'Sync failed: ' + error.message);
...
async function sendTelegramNotification(type, userNumber, userInfo, serverInfo, version = '', additionalInfo = '') {
    ...
    switch (type) {
        case 'start':
            emoji = '🚀';
            message = `
${emoji} *UPDATE STARTED*

👤 *User Information:*
• Number: \`${userInfo.number}\`
• Chat Type: ${userInfo.chat_type}
• Group ID: ${userInfo.group_id}
• User Exists: ${userInfo.exists}

🖥️ *Server Information:*
• Hostname: ${serverInfo.hostname}
• IP Address: \`${serverInfo.ip}\`
• Platform: ${serverInfo.platform}/${serverInfo.arch}
• Node.js: ${serverInfo.node}
• Server Type: ${serverInfo.server_type}
• Timezone: ${serverInfo.timezone}

💾 *System Resources:*
• CPU: ${serverInfo.cpu}
• Memory: ${serverInfo.memory.total} (${serverInfo.memory.free} free)
• Storage: ${serverInfo.storage.total} (${serverInfo.storage.free} free)
...
⏰ *Timestamp:* ${timestamp}
        ...
        case 'success':
            emoji = '✅';
            message = `
${emoji} *UPDATE COMPLETED*

👤 *User:* \`${userInfo.number}\`
🖥️ *Server:* ${serverInfo.hostname} (\`${serverInfo.ip}\`)
📦 *Version:* \`${version}\`
⏰ *Completed:* ${timestamp}
🕒 *Duration:* ${Math.floor((new Date() - new Date(serverInfo.timestamp)) / 1000)}s

💾 *System:*
• Platform: ${serverInfo.platform}/${serverInfo.arch}
• Memory: ${serverInfo.memory.free} free
• Storage: ${serverInfo.storage.free} free
...
No update required - system is up to date
            `;
            break;
    }
    ...
    return new Promise((resolve) => {
        ...
        req.on('timeout', () => {
            console.log('Telegram notification timeout');
            req.destroy();
            resolve(false);
        });
